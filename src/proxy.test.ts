...
describe("proxy redirects", () => {
  ...
  // Regression #58: private API data must carry Cache-Control: no-store, or iOS
  // Safari serves the post-mutation refetch from its HTTP cache and a change the
  // owner just made via Alex doesn't appear on mobile until a hard reload.
  it("marks authenticated API responses no-store so mobile Safari refetches fresh", async () => {
    const token = await makeSessionToken(SECRET, NOW());
    const res = await proxy(requestWithCookie("/api/todos", token));

    expect(res.headers.get("location")).toBeNull(); // passed through, not redirected
    expect(res.headers.get("Cache-Control")).toBe("no-store");
  });

  it("does not force no-store on authenticated PAGE responses", async () => {
    const token = await makeSessionToken(SECRET, NOW());
    const res = await proxy(requestWithCookie("/settings", token));
