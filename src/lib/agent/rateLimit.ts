...
// The once-per-identity booking guard. Kept in its own long-lived map, separate
// from the sliding message-rate window: bucketFor() below rolls buckets on
// WINDOW_MS and would otherwise zero this out, letting a conversation that
// outlives the window book again. This map never resets on its own — a given
// identity gets MAX_BOOKINGS, period. Callers must pass a server-controlled
// identity (e.g. IP), never an attacker-suppliable value like sessionId.
...
export function canBook(key: string, now = Date.now()): boolean {
  void now; // booking guard is not window-scoped; kept for signature compatibility
  return (bookingCounts.get(key) ?? 0) < MAX_BOOKINGS;
}
...
export function tryReserveBooking(key: string): boolean {
  const count = bookingCounts.get(key) ?? 0;
  if (count >= MAX_BOOKINGS) return false;
  bookingCounts.set(key, count + 1);
  return true;
}

/// Undo a reservation made by tryReserveBooking after the guarded write fails.
export function releaseBooking(key: string): void {
  const count = bookingCounts.get(key) ?? 0;
  if (count > 0) bookingCounts.set(key, count - 1);
}
