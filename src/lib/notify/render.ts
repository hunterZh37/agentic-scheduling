...
/// One host on the signature. `linkedin` is optional.
export interface SignatureHost {
  name: string;
...
export function renderBookingDescription(input: {
  ...
  videoUrl?: string;
  /// JOINT booking: the label to say the meeting is "with" (the team name), and
  /// every host to list in the signature. When set, these override the single
  ...
  // Who the meeting is "with": the team label for a joint booking, else the host.
  const withWhom = input.hostLabel?.trim() || hostFirst;
  const manageLine = input.manageUrl
    ? `Need to reschedule or cancel? Manage your booking here:\n${input.manageUrl}`
    : `It's on your calendar now. Need to reschedule or cancel? Just reply and ${withWhom} will sort it out.`;
  const lines = [
    `Hi ${first},`,
    ``,
    `Your ${minutes}-minute meeting with ${withWhom} is confirmed.`,
    ``,
    `When: ${day}`,
    `Time: ${window}`,
  ...
  // Joint booking: sign off from EVERY host, each with their LinkedIn. Solo: the
  // single owner, unchanged.
  if (input.hosts && input.hosts.length > 0) {
    for (const h of input.hosts) {
      lines.push(h.linkedin ? `${h.name} · ${h.linkedin}` : h.name);
    }
  } else {
...
export function renderBookingDescriptionHtml(input: {
  ...
  const withWhom = escapeHtml(input.hostLabel?.trim() || (input.hostName.trim().split(/\s+/)[0] || input.hostName.trim()));

  // Signature: every host on a joint booking (each with their LinkedIn), or the
  // single owner otherwise.
  let signature: string;
  if (input.hosts && input.hosts.length > 0) {
    signature = input.hosts
      .map((h) => {
        const nm = `<strong>${escapeHtml(h.name)}</strong>`;
        return h.linkedin
          ? `${nm} · <a href="${escapeHtml(h.linkedin)}">${escapeHtml(h.linkedin)}</a>`
          : nm;
      })
      .join("<br>");
  } else {
    const link = input.linkedinUrl
      ...
      : "";
    signature = `<strong>${escapeHtml(input.hostName)}</strong>${link}`;
  }

  return (
    `<div style="font-family:Arial,Helvetica,sans-serif;font-size:14px;line-height:1.5;color:#1a1a1a">` +
    `Hi <strong>${first}</strong>,<br><br>` +
    `Your <strong>${minutes}-minute</strong> meeting with <strong>${withWhom}</strong> is confirmed.<br><br>` +
