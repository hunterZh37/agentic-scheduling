...
export async function createBooking(
  ...
  const { busy, verified } = await revalidateBusyWithRetry(() =>
    fanOutBusy(input.start, input.end)
  );
  if (!verified) {
    throw new BookingError(
      "availability_unverified",
  ...
  if (!input.suppressAttendeeEmail) {
    await sendAttendeeConfirmation(booking).catch((err) =>
      console.error("[booking] attendee confirmation email failed:", err)
    );
  }
...
async function sendAttendeeConfirmation(booking: Booking): Promise<void> {
  if (!optionalEnv("RESEND_API_KEY")) return;
