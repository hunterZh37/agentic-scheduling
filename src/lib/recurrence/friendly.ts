...
export interface ParsedRule {
  ...
  byday: WeekdayCode[];
  bymonthday?: number; // e.g. -1 for the last day of the month
}
...
export function friendlyRecurrence(rule: string | null, isOvernight = false): string {
  ...
  if (freq === "MONTHLY") return bymonthday === -1 ? "Every month on the last day" : "Every month";
  // WEEKLY
...
export function detectPreset(rule: string | null, isOvernight = false): RecurrencePreset {
  ...
  if (freq === "MONTHLY") return bymonthday === -1 ? "monthlyLast" : "monthly";
  if (byday.length === 0) return "weekly";
  const set = new Set(byday);
  if (byday.length === 5 && WEEKDAYS_SET.every((d) => set.has(d as WeekdayCode))) return "weekdays";
  return "custom";
...
export function presetToRule(
  ...
  switch (preset) {
    ...
    case "weekly":
      return "FREQ=WEEKLY";
    // Anchored to the item's own day (dtstart / creation day) — "the 12th of
    // every month" needs no BYMONTHDAY when it repeats on its start day.
    case "monthly":
      return "FREQ=MONTHLY";
    // The last calendar day of every month, whatever its length.
    case "monthlyLast":
