...
vi.mock("@/lib/db", () => ({
  prisma: {
    account: { findUnique: vi.fn(), delete: vi.fn(), update: vi.fn(), updateMany: vi.fn() },
    $transaction: vi.fn(),
  },
}));
...
describe("DELETE /api/accounts/[id]", () => {
  it("deletes a non-destination account", async () => {
    vi.mocked(prisma.account.findUnique).mockResolvedValue({ id: "a1", isDestination: false } as never);
  ...
  it("refuses to delete the destination account (409) without deleting", async () => {
    vi.mocked(prisma.account.findUnique).mockResolvedValue({ id: "d", isDestination: true } as never);
