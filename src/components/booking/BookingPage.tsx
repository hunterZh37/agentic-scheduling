...
export function BookingPage({
  ...
  /// When set, this is a JOINT booking page for a team: availability and the
  /// booking write route through the team's slug, and the header shows the team.
  /// Unset = the owner's own booking page (unchanged).
  team?: {
    slug: string;
    name: string;
    eventTitle: string;
    ...
    /// Everyone on the team (owner first), each optionally linked to LinkedIn.
    members: { name: string; linkedin?: string }[];
  };
}) {
  // Where availability and the booking write go, and the identity shown — the
  // owner's own config, or the team's when this is a joint page. Deriving both
  // from one place keeps every call site below identical for the two modes.
  const availabilityPath = team
    ? `/api/availability/team/${team.slug}`
    : "/api/availability";
  const submitPath = team ? `/api/public/teams/${team.slug}/bookings` : "/api/public/bookings";
  const eventTitle = team?.eventTitle ?? HOST.eventTitle;
  const headerName = team?.name ?? HOST.name;
  const videoLink = team ? team.videoLink ?? undefined : HOST.videoLink;
  const durationOptions = team?.durationOptionsMinutes ?? HOST.durationOptions;
  const defaultDuration = team ? team.durationOptionsMinutes[0] : HOST.durationMinutes;
  ...
  useEffect(() => {
    ...
    fetch(`${availabilityPath}?start=${encodeURIComponent(dayStart!)}&end=${encodeURIComponent(dayEnd!)}&duration=${duration}`)
      .then((r) => r.json())
  ...
  if (step === "success" && confirmed) {
    return (
      <div className={styles.page}>
        <SuccessCard confirmed={confirmed} fmtRange={fmtRange} onReschedule={reset} rescheduled={!!reschedule} eventTitle={eventTitle} videoLink={videoLink} />
      </div>
    );
  }
  ...
  return (
    <div className={styles.page}>
      <div className={styles.window}>
        ...
        <div className={styles.body}>
          ...
          <aside className={styles.rail}>
            {/* The owner's photo is their solo identity — omit it on a joint
                page, where more than one person is hosting. */}
            {!team && (
              <img className={styles.avatar} src="/avatar.jpg" alt={HOST.name} width={64} height={64} />
            )}
            <div className={styles.hostName}>{headerName}</div>
            {/* What this booking is FOR. A visitor arriving from a cold link has
                ...
                scanners look for. The owner's solo consulting identity does not
                belong on a JOINT page, so it is owner-page only. */}
            {!team && (
            ...
            {/* The practice's areas, one per line. This is the rail's one
                ...
                never claim different expertise in different places. On a team
                page it becomes "who you're booking with" — the members whose
                calendars this link reconciles. */}
            <div className={styles.railLabel}>
              {team ? "Who you're booking with" : "The areas I consult in"}
            </div>
            <ul className={styles.areas}>
              {team
                ? team.members.map((m) => (
                    <li key={m.name} className={styles.area}>
                      <span className={styles.areaDot} aria-hidden="true" />
                      {m.linkedin ? (
                        <a
                          className={`${styles.areaText} ${styles.memberLink}`}
                          href={m.linkedin}
                          target="_blank"
                        ...
                        <span className={styles.areaText}>{m.name}</span>
                      )}
                    </li>
                  ))
            ...
            {/* Authorship credit. Deliberately quiet (small, muted label, set
                ...
                the domain's whois. */}
            {/* Owner-page only: on a JOINT page the meeting isn't "built by" one
                member, so the personal authorship credit is dropped (the
                open-source link below stays). */}
            {!team && (
          ...
          <section className={styles.scheduler}>
            {reschedule ? (
              ...
              <div className={styles.switcher} role="tablist">
                ...
                >
                  Ask Alex
                  <span className={styles.tabBadge} aria-hidden="true">AI</span>
                </button>
              </div>
            )}
            ...
            {mode === "agent" && !reschedule ? (
              <div key="agent" className={styles.panel}>
                <PublicAgentChat
                  ...
                  endpoint={team ? `/api/agent/public/team/${team.slug}` : undefined}
                  intro={
                    team
                      ? `Hi! I can help you find a time that works for ${team.name}. When are you hoping to meet?`
                      : undefined
                  }
                />
              </div>
      ...
      {/* Visible Privacy/Terms footer removed at the owner's request. The
          reputation-facing legal links still ship server-side in the page's
          <noscript> (see app/book/page.tsx), and the owner-identity trust signal
          lives in the rail's "Built by" credit — so this removal is visitor-only,
          not a reputation regression. */}
    </div>
  );
}
...
function SuccessCard({
  ...
  onReschedule: () => void;
  rescheduled?: boolean;
  /// Event identity, from the owner's config or the team's (joint page).
  eventTitle: string;
  videoLink?: string;
}) {
  ...
  const gcal = () => {
    ...
    if (videoLink) url.searchParams.set("location", videoLink);
  ...
  return (
    <div className={styles.successCard}>
      ...
      <div className={styles.successSummary}>
        <div className={styles.successEvent}>{eventTitle.toUpperCase()}</div>
        ...
        {!rescheduled && videoLink && (
          <div className={styles.successVideo}>
            Video call:{" "}
            <a href={videoLink} target="_blank" rel="noreferrer">
              {videoLink.replace(/^https?:\/\//, "")}
            </a>
          </div>
        )}
      </div>
