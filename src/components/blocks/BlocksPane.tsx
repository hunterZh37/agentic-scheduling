...
export function BlocksPane({ blocksOverride, bookingsOverride, eventsOverride, selectedDate, onScheduleChange, reloadKey = 0, active = true }: BlocksPaneProps) {
  ...
  const [pendingTodoDelete, setPendingTodoDelete] = useState<{ id: string; title: string } | null>(null);
  ...
  const reload = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [b, k, s, t, c, f] = await Promise.all([
  ...
  const deleteTodo = async (id: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        // 404 is the interesting one: the row the UI is showing no longer
        // exists server-side, i.e. the list is stale rather than the delete
        // being broken.
        setActionError(
          res.status === 404
            ? "That actionable was already gone — refreshing the list."
            : `Couldn't delete that actionable (${res.status}). It's still there.`
        );
      } else {
        setActionError(null);
      }
    } catch {
      setActionError("Couldn't reach the server to delete that actionable.");
    }
    void reload();
  };
    ...
    <div className={styles.root}>
      <div className={styles.header}>
        <div className={styles.headerLeft}>
          <h2 className={styles.headerTitle}>Blocks</h2>
              ...
              {untimedTodos.map((t) => {
                ...
                <li key={`todo:${t.id}`} className={`${styles.row} ${t.done ? styles.rowChecked : ""}`}>
                  ...
                  >
                    ...
                    {(t.location || t.videoLink || t.phone || t.rolledFromId) && (
                  ...
                  <button
                    ...
                    onClick={() => setPendingTodoDelete({ id: t.id, title: t.title })}
                ...
                return (
                  ...
                  >
                    ...
                    {isTodo && (
                      <button
                        className={styles.rowDelete}
                        onClick={() => setPendingTodoDelete({ id: item.todoId!, title: item.title })}
              ...
              {upcomingBookings.map((bk) => {
                ...
                <li key={bk.id} className={`${styles.row} ${cancelled ? styles.rowCancelled : ""}`}>
                  ...
                  >
                    ...
                    <span className={styles.rowSub}>
                      <span className="tnum">
                        {relativeDayTime(new Date(bk.startTime), bk.attendeeTimezone)}
                      </span>
                      <span className={styles.dot}>·</span>
                      {bk.attendeeTimezone}
                      <span className={styles.dot}>·</span>
      ...
      {pendingTodoDelete && (
        <ConfirmDialog
          title={`Delete “${pendingTodoDelete.title}”?`}
          body="This removes the actionable from your list. This can’t be undone."
          confirmLabel="Delete actionable"
          busy={deletingTodo}
          onCancel={() => setPendingTodoDelete(null)}
          onConfirm={async () => {
            setDeletingTodo(true);
            await deleteTodo(pendingTodoDelete.id);
