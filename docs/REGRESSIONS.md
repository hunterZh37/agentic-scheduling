...
| 61 | "People can book time that should have been reserved for carried over actionables" | An unfinished timed actionable is copied to the next day at the same time-of-day, daily, until done — but the copy only becomes a `Todo` row on the day itself (the 14:00 UTC / 7 AM Pacific carry-forward cron). `actionableBusy` counted only rows that EXIST, and visitors book days ahead, so the slot the actionable was about to occupy was offered as free; the cron later dropped the actionable on top of the booking. Same hole overnight: between local midnight and the cron, yesterday's unfinished actionable did not block today. Reproduced against the local DB with the real function: tomorrow 3pm returned 0 busy intervals with an unfinished 3pm today | `test` `src/lib/availability/carryProjection.test.ts` — the live end of each carry chain (unfinished, timed, on today or yesterday, not yet carried) reserves its time-of-day on the following `CARRY_PROJECTION_DAYS` (7) days; never its own day (the real row covers it); DST keeps the local time and the duration; a finished or already-carried actionable projects nothing. Wired inside `actionableBusy`, the one function every busy path shares (`busySources.test.ts`), so the slot list and the write guard cannot disagree |
| 64 | "Why is the sleeping time able to be booked?" — /book offered midnight to 6:30 AM and 11 PM while the Reserved list showed "Sleep, 11:00 PM – 7:00 AM, Every night" | The production Sleep block's rule was `FREQ=DAILY;UNTIL=20260103T075959Z` (an end date set during a January edit), so it had stopped repeating on Jan 3 and reserved nothing; the label only read the frequency, so the list kept saying "Every night" for eight months. Every other block had no UNTIL and worked. Found by reading `/api/blocks` through the owner's browser session; the local data copy was stale and correct, which is why the pipeline reproduced fine locally | Data fixed in place (rule set back to `FREQ=DAILY`; Sept 28 went from 29 slots to 14, none before 11 AM or at 11 PM). `test` `friendly.test.ts` — a rule with a past UNTIL is labelled "… · ended Jan 2, 2026" (the day in the block's own zone, the day the owner picked) and `recurrenceEnded` is true; a future UNTIL reads "· until <date>"; a COUNT that has run out is ended on its last occurrence, computed with the same rrule expansion the engine uses. The list paints an ended rule in the warning colour with a hover explanation |
...
**Watch:** the OWNER's public availability is scoped to the owner's own data —
`coHostId: null`. Accounts (`fanOutBusy`) and reserved blocks
(`availability/service.ts`) both filter on it. A co-host's connected calendar or
reserved block carries a NON-null `coHostId`, and if it ever leaked into the
owner's free/busy it would silently shrink (or, for a wide block, erase) the
owner's bookable time — the same class as #1/#2 where one bad busy span took the
booking page down, but arriving via a *different person's* data. The joint
(team) availability engine must query specific co-host ids on purpose, never by
dropping this filter. Guards: `test` `src/lib/calendar/aggregate.test.ts` (owner
= `coHostId:null` by default) + `src/lib/availability/service.test.ts` (blocks
scoped to `coHostId:null`).

## Joint (team) booking

A team link `/book/<slug>` offers only times EVERY member (owner + co-hosts) is
free, and one booking puts everyone on a single invite.

**Watch:** the joint write must re-check **every** member at book time, not just
the owner. `createBooking` fans out `fanOutBusy` per subject (`[null,
...coHostIds]`) and fails closed if any is busy or unverifiable — hiding a slot
on the page is never enough, because a direct `POST /api/public/teams/<slug>/
bookings` reaches the write without loading the page. Guard:
`src/lib/booking/jointBooking.test.ts` (co-host busy → rejected, unverified →
`availability_unverified`, never written).

**Watch:** the co-host lands on the event by being an ATTENDEE, not by a second
calendar write. There is exactly ONE write, to the owner's destination account,
with the co-host emails in `additionalAttendeeEmails`; the provider then shows it
on their calendar via the invite. Writing a second event with the co-host's own
tokens would double-book them. Guard: same test asserts the draft carries the
co-host emails and only the destination is written.

**Watch:** the owner's OWN dashboard (`getScheduleView`) must also scope to
owner-only — `account.findMany({where:{coHostId:null}})` and
`personalBlock.findMany({where:{visible:true,coHostId:null}})`. Both queries
shipped unscoped, so a co-host's connected calendars and reserved blocks would
have appeared on the owner's calendar grid — the owner side of the privacy wall,
the mirror of the availability scoping. Guard: `src/lib/schedule/service.test.ts`
("scoped to the owner"). (This surfaced when a co-host's event showed on the
owner's dashboard while the owner debugged joint availability — the joint
availability itself was correct; a co-host busy in a bookable window does drop
from the team page, verified end-to-end.)

**Watch:** joint availability and the joint write both scope blocks/accounts to
the team's members only — owner (`coHostId: null`) plus the listed co-host ids.
A slot must never be removed by a NON-member's busy time, and the owner's own
`/book` must never be affected by a co-host (see the Booking availability
scoping note). Guards: `jointService.test.ts`, `jointBooking.test.ts`.

**Watch:** the co-host pages (`/cohost`, its calendar list) are inline-styled and
were shipped with hard-coded light-theme hex (`#1d1d1f` text, `#fff` cards). The
dashboard runs DARK, so near-black text on the dark ground was unreadable — the
owner reported "text are unreadable." Colours on these pages MUST use the theme
tokens (`var(--text-1/2/3)`, `var(--surface)`, `var(--hairline)`, `var(--accent)`,
`var(--state-cancelled)`), never raw hex, so they read in both themes. Guard:
`manual` — visual; `/cohost` is behind the co-host session gate, so the
public-pages mobile audit never loads it.

**Watch:** the "Ask Alex" assistant on a team page MUST run the team-scoped
tools, not the owner's. `runTeamAgent` wires `get_availability` to
`getJointAvailability` and `create_public_booking` to `createBooking` with the
team's coHostIds/emails/teamId — so the agent can only ever offer times everyone
is free and book onto the joint link. The single-host `runPublicAgent` (owner
availability) must never be reachable from `/api/agent/public/team/<slug>`.
Guard: `proxy.test.ts` lists the endpoint as public; the tool wiring is in
`src/lib/agent/run.ts` / `tools.ts`.

**Watch:** `/book/<slug>`, `/api/availability/team/<slug>`,
`/api/public/teams/<slug>/bookings` and `/api/agent/public/team/<slug>` are public. Like every visitor route they
must stay in `PUBLIC_PREFIXES` (they ride on `/book`, `/api/availability`,
`/api/public/`), and `/book/<slug>` must keep server-rendered `<noscript>`
identity for reputation crawlers — same rule as `/book`. Guard:
`proxy.test.ts` (the three routes resolve without a session).
...
**Watch:** a label that summarises only part of a rule will lie about the
rest. Anything shown to the owner about a recurrence must include its end
(`UNTIL`/`COUNT`), or the owner cannot tell an active block from a dead one.
And the local database is a COPY: when local reproduces fine, read the live row
through the owner's session before trusting the code.
...
| # | Symptom | Cause | Guard |
|---|---|---|---|
| 59 | On a phone, the Alex conversation can't be scrolled all the way to the bottom — the last message(s) sit behind the input composer | The composer floats over the thread (`position: absolute; bottom: 0`) and its padding reserves `env(safe-area-inset-bottom)` (~34px on a notched iPhone). The thread's scroll reservation was a **fixed** `calc(var(--space-6) + 44px)` that omitted that inset, so the composer was ~34px taller than the thread reserved and the last ~34px of content hid behind it. `env()` is 0 on desktop / non-notched devices, so it was invisible there and the headless mobile audit (which runs with a 0 inset) can't see it | `test` `src/components/agent/agentPaneLayout.test.ts` — asserts the invariant that whenever the floating composer reserves `env(safe-area-inset-bottom)`, the scroll thread must reserve it too (`.thread` bottom padding now includes the inset). The visual clipping is device-only; this structural tie between the two rules is the guardable part |
| 58 | On mobile, Alex says it added an event/actionable or deleted a meeting ("done"), but nothing appears — a hard reload shows it, and desktop was fine the whole time | The write always succeeded (Alex's "done" is produced only after the tool result). The private data GETs (`/api/todos`, `/api/schedule`, `/api/blocks`, `/api/bookings`, `/api/followups`, `/api/recurring`, ...) sent **no `Cache-Control`**. After a mutation the panes refetch the same URLs, but **iOS Safari served those GETs from its own HTTP cache**, returning pre-mutation data; a hard reload bypasses the cache, so the item appears. Desktop Chrome doesn't heuristically cache these, so the staleness was invisible there. Not the Sonnet 5 switch (desktop uses the same model) | `test` `src/proxy.test.ts` — the auth proxy stamps `Cache-Control: no-store` on authenticated `/api/` responses at a single choke point, so every browser (Safari included) refetches fresh after a mutation; a page response is asserted to keep its normal caching. The Safari-cache behavior itself is device-only and can't be seen by unit tests or the headless audit — the header is the guardable invariant |
| 57 | On a phone, the booking page's "Ask Alex" chat input pill had its bottom cut off | Different component from #56 — this is the PUBLIC booking chat (`PublicAgentChat` in `BookingPage.module.css`, not the dashboard). `.chatComposer` had no bottom padding, so the input pill sat flush against the `.window`'s rounded `overflow: hidden` bottom edge (and iOS Safari's bottom toolbar), clipping it. Not reproducible headless (renders with ~20px slack; a real device's rounded corner + toolbar eat it) | `.chatComposer` gets `padding-bottom: calc(var(--space-4) + env(safe-area-inset-bottom))` at `max-width: 768px`, clearing the window edge and the toolbar. Guard `manual` — device chrome / rounded-corner clipping isn't visible to the headless audit |
| 56 | "The bottom of the Alex input box is cut off" (on a tablet-width browser) | `.main` used `height: 100vh` and only switched to `100dvh` at `max-width: 768px`. On a tablet/laptop browser WIDER than 768px, a retractable bottom toolbar makes `100vh` the toolbar-hidden height, so the bottom-anchored agent composer is drawn behind the toolbar and its bottom is clipped — the exact case the phone-only `100dvh` was added to prevent, just above its width cutoff. Not reproducible in headless Playwright (no browser chrome to simulate) | `.main` now uses `height: 100vh; height: 100dvh;` at every width (dvh tracks the visible viewport; the vh line is the pre-dvh fallback). Guard `manual` — dynamic-viewport / browser-chrome behavior can't be seen by unit tests or the headless mobile audit |
| 55 | The recurring-schedule detail/edit modal (click a Recurring row) had sub-44px controls and a 14px Repeat select on phones | It reuses EventModal's edit-form classes (`.fInput`, `.btn*`), which had no phone tap-target sizing — the mobile audit never caught them because it opens EventModal in VIEW mode only and never enters edit mode, so those classes were unmeasured. The recurring editor shows them immediately, exposing it | `.fInput`/`.btn*` clear 44px and `select.fInput` is 16px on phones (`EventModal.module.css`), fixing the recurring editor AND EventModal's own edit mode. Verified with a targeted iPhone-SE measurement of the modal (fits, no overflow, no small taps/inputs). NOTE: the audit still can't reach these automatically — an edit-mode overlay scenario would need seed data guaranteeing an editable row |
| 54 | Mobile audit reported the add-card / delete-dialog / event-modal overlays "could not open" — timeouts, not layout faults | The overlay scenarios switch to a phone tab by NAME (`getByRole("tab", {name: /Blocks/i})`), but that tab was renamed **"Blocks" → "Schedule"**. The match silently found nothing, so the tab was never switched, the pane's controls stayed `display:none`, and every `scenario.open` click timed out — a guard passing (or, here, failing for the wrong reason) while testing nothing. Fixing the tab name then surfaced PRE-EXISTING event-modal issues the broken guard had masked: a raw location URL overflowing 28px, and `.close`/`.editTop`/`.openExternal`/`.locationLink`/follow-up input under the 44px tap floor | `audit:mobile` uses the live tab name (`Schedule`); the modal chrome clears 44px on phones and the location link wraps (`EventModal.module.css`); links inside a provider description (`.desc`, `dangerouslySetInnerHTML`) are exempted as prose, like inline legal-copy links; the Repeat `<select class="chip">` was also bumped to 16px so iOS no longer zooms on focus (`.chip` out-specified the global `select` anti-zoom rule) |

**Watch:** the mobile audit's overlay scenarios click a tab BY ITS VISIBLE NAME.
Renaming a dashboard tab (as "Blocks" → "Schedule") silently breaks the scenario
without failing loudly — it times out on the open click instead. Any tab rename
must be mirrored in `OVERLAYS[].tab`.

**Watch:** a component `.chip`-style class sets its own `font-size` and will
out-specify the global `@media (max-width:768px) select { font-size:16px }`
anti-zoom rule (element selector, lower specificity). A `<select>` carrying such
a class needs the 16px re-asserted at class level, or iOS zooms on focus.
...
**Watch:** the OWNER's dashboard shows every time in `OWNER_TIMEZONE`. The
"Upcoming bookings" row once rendered a booking in the ATTENDEE's zone
(`bk.attendeeTimezone`) while the calendar grid and the event-detail modal used
`OWNER_TIMEZONE` — so the same booking read "8:15 PM America/Chicago" in the
agenda and "6:15 PM America/Los_Angeles" in the detail, on the owner's own
screen. Anything the owner sees on their dashboard uses `OWNER_TIMEZONE`; the
attendee's zone belongs on the attendee's confirmation/invite, not the owner's
views. Guard: `manual` — visual (BlocksPane "Upcoming bookings" row).
...
| 60 | "When I click on that link, it redirects me to google map instead of zoom. Need ability to hold either online meeting link or google map link" — a Kaiser video visit whose location read `[Zoom video visit](https://healthy.kaiserpermanente.org/…)` | `locationHref` only treated a location as a link when the WHOLE string started with `http`/`www.`. Providers also write the meeting link as markdown (`[label](url)`) or after text (`Zoom: https://…`); the URL then sits mid-string, so it fell through to a Google Maps search for the literal markdown. The panel also showed the raw markdown as the link text | `test` `src/lib/maps.test.ts` — the owner's exact Kaiser string resolves to its URL; text-then-URL and trailing punctuation are handled; addresses still go to Maps; a non-http scheme (`javascript:`) never becomes an href, since location text comes from other people's invites. `locationLabel` shows a markdown link as its label, and `isMeetingLocation` swaps the map pin for the video icon |
| 62 | "Why does Tech for IOG fellowship's time not get displayed?" then, on opening it, "It did have the time from 9 to 9:30" — an actionable with NO time showed "9:00 – 9:30 AM · America/Los_Angeles" in the detail panel | `untimedTodoDetailItem` seeded a 9:00 + 30 min slot into the item so the editor had a prefill (row for the 2026-08-19 "untimed rows can't be opened" fix), and the panel's read view rendered that placeholder exactly like a stored time. Worse, a title-only edit then saved the placeholder as a real time. Every untimed actionable claimed 9:00–9:30 | `test` `detailItem.test.ts` (item is `untimed`, anchored to the day at midnight, no time invented) and `whenRow.test.ts` (the panel shows "No time set" with no digits for an untimed item; the editor refuses one-sided times instead of dropping or defaulting them; both empty keeps it untimed). Reminders are hidden for untimed items |

**Watch:** a placeholder that looks like data IS data to the reader. Anything
seeded only to give an editor a prefill (a default time, a sample address)
must be flagged on the item and hidden by every read view, or it will be
believed and then saved.

**Watch:** a location field holds a PLACE or a MEETING LINK, and providers write
the link in more shapes than a bare URL (markdown, "Zoom: https://…"). Anything
that renders a location goes through `src/lib/maps.ts` (`locationHref`,
`locationLabel`, `isMeetingLocation`) — never build a Maps URL at the call site.
...
day**, so `now.setZone(OWNER_TIMEZONE).startOf("day")` resolved to *yesterday*
and the carry landed a full day behind (on Aug 28, unfinished Aug-27 items were
never moved to Aug 28). Moved to `0 14 * * *` (7 AM PDT / 6 AM PST). Rule: this
cron's UTC hour MUST stay past owner-local midnight — re-check it if
`OWNER_TIMEZONE` changes or the owner moves further west (Pacific midnight is
7 UTC in summer, 8 UTC in winter). Guard: `manual` — cron timing vs a
production env var is exactly the class unit tests can't see.

**Watch (recurring actionables):** a `RecurringTodo` is a TEMPLATE; the same
daily cron that carries forward now also *materializes* each active template's
due days into ordinary `Todo` rows (`materializeRecurringTodos`, run BEFORE the
carry-forward). Three invariants, all guarded in `src/lib/todos/recurring.test.ts`:
(a) **Never backfill.** A brand-new template (`lastMaterializedOn` null) seeds
only *today*, never its past occurrences — otherwise "pay rent, last day of every
month" created today would spray a row onto every month-end since the anchor.
(b) **Idempotent seeding.** `@@unique([recurringTodoId, date])` is the real guard
(mirroring `rolledFromId`): one template seeds at most one row per day, so a cron
retry or an overlapping catch-up window is a safe no-op (`P2002` swallowed). Do
NOT weaken this to an in-code "already seeded?" check — the same lesson as the
carry-forward unique constraint. (c) **Owner-timezone days.** rrule is
timezone-naive, so occurrences are driven through "naive-UTC" dates and mapped
back to owner-local day-keys; "last day of the month" and DST both depend on it.
A seeded row is an ordinary actionable afterwards — it carries forward and nags
like any other, which is intended (an unpaid rent should keep reappearing).

| # | Symptom | Cause | Guard |
|---|---|---|---|
| 53 | "there should be UI/UX for adding recurring events/actionables" — recurring actionables could only be created by chatting with Alex | The quick-add's **Repeat** dropdown was hidden for the Actionable type (`itemType !== "todo"`), and `addItem`'s todo branch posted a one-off to `/api/todos`, silently dropping any recurrence. It also had no monthly presets, so "pay rent, last day of month" was inexpressible in the UI at all. Events/blocks already recurred | Repeat is now shown for every type; Actionable + a repeat routes to `POST /api/recurring` (new), which shares `createRecurringActionable` with the agent tool so chat and UI behave identically; added **Every month** / **Monthly on the last day** presets (actionables + events only — the block expander's whole-week anchor shift would break a monthly phase, so blocks are excluded). `test` `friendly.test.ts` (preset↔rule↔label round-trip incl. monthly), `recurring.test.ts` (`createRecurringActionable` validates + seeds). Live-verified both untimed-monthly and timed-weekly seed the right day. Visual guard `manual` |
| 52 | "I had a recurring monthly item pay rent, but it did not show up" / "the recurring actionable did not show up on my schedule UI" | Not a materialization bug — reproduced against the code (`_repro`): a `FREQ=MONTHLY;BYMONTHDAY=-1` series created on the 30th correctly seeds nothing until the 31st. The real fault was the SCOPE shipped: a recurrence was surfaced only as a badge on the *seeded* to-do, which appears only on its due day. A series created before its first due day had zero UI presence, so "set up but not due yet" looked identical to "silently failed to create". The owner set one up and saw nothing | `test` `src/lib/todos/recurring.test.ts` covers `nextOccurrence`/`describeRRule`; the fix has two halves. (1) A **Recurring** section in the Blocks pane (shown when any active `RecurringTodo` exists) listing each series with its cadence and next due day, fed by `GET /api/recurring`, cancellable via `DELETE /api/recurring/[id]`. (2) Follow-up "I want this to show up as an actionable" — a recurring to-do must be a REAL actionable on its due day, not just a schedule row. `materializeTemplate` now **eagerly seeds the next upcoming occurrence** (not only today's), so setting up "pay rent, last day of month" on the 30th puts a real Pay-rent actionable on the 31st immediately (`recurring.test.ts` covers this). And `GET /api/recurring` runs `materializeRecurringTodos()` before listing — idempotent, owner-only — so opening the dashboard self-heals a series created before the eager seeding shipped (or a day the cron missed). NOTE: all of this needs the `add_recurring_todo` migration applied to prod — an empty section despite a set-up series means the row was never written (migration missing), not a render bug |
...
| 65 | "I cannot click on the link" — a to-do item reading "Open in Celeste: https://…" showed the URL as plain text | `renderInline` (shared by to-do items, the agent chat panes and the booking chat) only turned markdown `[label](url)` into anchors; a bare http(s) URL, which other agents and people paste far more often, stayed text | `test` `src/components/agent/markdown.test.tsx` — a bare URL becomes an anchor with the surrounding text intact, trailing sentence punctuation and an unbalanced closing paren stay outside the link, markdown links are not double-linked, and no unsafe scheme is ever linked. vitest now includes `.test.tsx` |
